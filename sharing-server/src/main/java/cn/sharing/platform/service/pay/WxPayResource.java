package cn.sharing.platform.service.pay;
...
import cn.sharing.platform.facade.pay.v1.PayFacade;

import cn.sharing.platform.facade.payment.v1.PayInfoParam;
import cn.sharing.platform.service.pay.wxpay.*;
import cn.sharing.platform.service.Payment.PaymentServiceImpl;
...
@RestController
public class WxPayResource implements PayFacade {
...
//		return payout;
	...
	@Override
	public ResponseResult<PayOut>  Query(@PathVariable(value = "payId") @ApiParam(name = "payId", value = "支付代码") String
								payId) {
//
...
//		return payout;
	...
	@Override
	public ResponseResult<PayOut> Refund(@RequestBody  PayIn preorder) {
		PayOut payout = new PayOut();
		WxAccount wxaccount = new WxAccount();
		wxaccount.setAppId(wxPayConfig.getAppid());
		wxaccount.setAppkey(wxPayConfig.getKey());
		...
		if(payout != null && payout.getResult_code().equals("S")){
			return  new ResponseResult<>(payout);
		}else{
			return ResponseResult.failed(payout.getResult_msg());
		}
	}
	@Override
	public ResponseResult<PayOut>  Cancel(@RequestParam("out_trade_no") String out_trade_no) {
...
//			}
			if("SUCCESS".equals((String) map.get("refund_status"))){
				...
				paymentServiceImpl.save(payInfoParam);
				try {
					borrowServiceImpl.RefundSuccess((String) map.get("out_trade_no"), payId);
				}catch (Exception e){
	...
	public void Notify(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		String notityXml = sb.toString();
		String resXml = "";
		LOGGER.info("接收到支付回调的报文：" + notityXml);
...
//			}
			...
			payInfoParam.setOutTradeNo((String) map.get("out_trade_no"));
			payInfoParam.setPayType(1);
			payInfoParam.setPayAmt(new BigDecimal((String) map.get("total_fee")).divide(new BigDecimal(100)).setScale(2));
			...
			paymentServiceImpl.save(payInfoParam);
			try {
				borrowServiceImpl.PaySuccess((String) map.get("out_trade_no"), payId);
			}catch (Exception e){
				LOGGER.info("支付订单回写失败" + (String) map.get("out_trade_no") + "_" + payId );
			}
