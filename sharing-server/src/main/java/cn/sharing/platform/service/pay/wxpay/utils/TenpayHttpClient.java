package cn.sharing.platform.service.pay.wxpay.utils;
...
public class TenpayHttpClient {
	 ...
	 * @param certLessFile
	 */
	public void setCertLessFile(File certLessFile) {
		this.certLessFile = certLessFile;
	}
	...
	public boolean call() {
		...
		//http
		if(null == this.caFile && null == this.certFile && null == this.certLessFile) {
			try {
				this.callHttp();
				isRet = true;
			} catch (IOException e) {
	...
	protected void callHttp() throws IOException {
		...
		if("POST".equals(this.method.toUpperCase())) {
			...
			System.out.println(reqXmlPosData);
			byte[] postData = reqXmlPosData.getBytes(this.charset);
	 ...
	 */
	protected void callHttps2() throws Exception {
		
		KeyStore keyStore  = KeyStore.getInstance("PKCS12");
		FileInputStream instream = new FileInputStream(this.certLessFile);
		System.out.println(this.certLessFile);
		try {
            keyStore.load(instream, mchid.toCharArray());  //Modified by liweihua  mchid 从配置里面读改为本类的变量
        } finally {
            instream.close();
        }
		...
		System.out.println("aa111");
		 try {
			 System.out.println(this.reqContent);
			 String url = HttpClientUtil.getURL(this.reqContent);
	...
	public String getReqXmlPosData() {
		return reqXmlPosData;
	}

	public void setReqXmlPosData(String reqXmlPosData) {
		this.reqXmlPosData = reqXmlPosData;
	}
